...
class ManagePartnersController extends BaseAdminController {

    protected $layout = 'layouts.full';


    public function getIndex() {
        return View::make('pages.managePartners.company');
    }
    ...
    public function getCompanyRegister($companyID = null) {
        if (Auth::check() && Auth::user()->role == 1) {
            if (empty($companyID)) {
                return View::make('pages.managePartners.company_form');
            } else {
                $companyDetails = Company::getCompanyDetailByCompanyID($companyID)->toArray();
                return View::make('pages.managePartners.company_form')->with('companyID', $companyID)
    ...
    public function getSiteRegister($companyID = null, $siteID = null) {
        if (Auth::check() && !empty($companyID)) {
            ...
            if(!empty($siteID)){
                return View::make('pages.managePartners.site_form')->with('companyID', $companyID)->with('data', $siteInformation);
            }else{
                return View::make('pages.managePartners.site_form')->with('companyID', $companyID);
            }
        } else {
            return App::abort(403, 'Unauthorized action.');
        }
