		...
		return {
			...
			template: function(element, attrs) {
				...
				var template = 
					...
					'		<i class = "black inverted photo icon"></i>'+
					...
					'				<div class="background-image-selector-upload-preview-actions ui vertical buttons">'+
			...
			link:function($scope,element,attr,ngModel){
				...
				*/
				...
				function getImagePaths(siteID){
					$http.get(rootScope+'/imageFolder/images/'+$scope.pageFileName+'/'+siteID)
				...
				*/
				...
				reader.onload = function (event){
					...
					ngModel.$setViewValue('url(../../'+event.target.result+')');
				...
				element.find('.background-image-selector-url-input input').bind('change', function(){
					...
					ngModel.$setViewValue('url(../../'+$(this).val()+')');
				});
				
				element.find('.background-image-selector-upload-preview-save').bind('click', function(){
					
					$http.post('imageFolder/upload-image', {'siteID': siteID, 'image': $scope.uploadImage, 'pageFileName': pageFileName}).success(function(data) {
						//console.log(data);
				...
				element.find('.background-image-selector-remove').bind('click',function() {
					$http.post('imageFolder/delete-image', {'siteID': siteID, 'pageFileName': pageFileName, 'image': element.find('.active-image').attr('src')}).success(function(data) {
						console.log(data);
						//alert(data);
						getImagePaths(siteID);
					});
