...
(function() {
    ...
    dropdownListDirective.directive('dropdownList', function($compile) {
        return {
            template: 
                '<div class="ui {{settings.dropdownClass}}">' +
                '   <input type="hidden" name="{{settings.name}}" id="{{settings.id}}">' +
                '   <div class="default text" ng-if ="ngModel!=null && ngModel!=\'\'" ng-repeat = "' + list + ' | filter:{' + listObjectName + ':' + ngModelName + '}" ' + dropdownStyle + ' ' + dropdownCustomAttributes + '">{{' + listDisplay + '}}</div>' +
                ...
                '   <div class="menu">' +
                '       <div class="item" ng-repeat = "' + list + '" data-value="{{' + listValue + '}}" ' + listStyle + ' on-finish-render="ngRepeatDropdownListFinished">{{' + listDisplay + '}}</div>' +
                '</div>',
            restrict: 'E',
            replace: true,
            scope:{
            ...
            link: function(scope, element, attr, ngModel) {
                scope.$on('ngRepeatDropdownListFinished', function() { 
    ...
    */
    ...
    dropdownListDirective.directive('onFinishRender', ['$timeout', '$compile', function (timeout, compile) {
        return {
            restrict: 'A',
